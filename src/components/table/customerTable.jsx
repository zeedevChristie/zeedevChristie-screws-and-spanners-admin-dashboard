...
const columns = [
  {
    ...
    headerAlign : "center",
    renderCell: (params) => (
      <div style={{ textAlign: "center", color: "blue" }}>{params.value}</div>
    ),
  },
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setCustomers(data.customers); // Update the rows state with the customers array
      } catch (error) {
    ...
    <div className="px-4 h-[63vh] w-[84vw] overflow-auto">
      <div className="flex justify-between p-2">
        ...
        <Link to="/dashboard/createNewCustomer">
          <h2 className="border-2 bg-green-200 rounded-lg p-2">
            Create New Customer
          </h2>
        </Link>
      </div>
      ...
      <DataGrid
        rows={filteredCustomers} // Use rows state to display customer data
        columns={columns}
        initialState={{
